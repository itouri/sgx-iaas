...
// 正確には client ってよりは master_enclave.cpp って名前が正しい
...
#define MODE_MRENCLAVE 0x3 //added
...
int main (int argc, char *argv[])
{
	...
	static struct option long_opt[] =
	{
		...
		{"mrenclave",	required_argument,	0, 'g'}, //added arg = target_enclave_id
		{ 0, 0, 0, 0 }
	};
	...
	} else if ( config.mode == MODE_MRENCLAVE ){
		get_mrenclave(eid, &config);
...
int do_attestation (sgx_enclave_id_t eid, config_t *config)
{
	...
	if ( config->server == NULL ) {
		// 引数なしの場合 stdout, stdinとread/write
		msgio = new MsgIO();
	} else {
		try {
			// 引数ありの場合，ネットワークコネクション
	...
	//Task service provider の公開鍵はこのコードにハードコーディングする
	...
	// ここでmsg0&1を送ってる
	// partialとは 分割送受信
	/* ファイル書き込み fplogはlogファイルへのファイルポインタ */
	...
	// sendとsend_partialの違いはなんだ？
	// sendは実際に送信するけど send_partial は buffer に追加するだけで送信は行わない
	...
	//TODO sgxrvってなんだ？
	//enclave_launch_vm(eid, &sgxrv, ra_ctx);
